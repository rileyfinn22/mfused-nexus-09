        ...
        <div style={{ width: 420, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 30, marginTop: 30 }}>
            {[{ n: "12", label: "Total Files" }, { n: "7", label: "Approved" }, { n: "10", label: "Products" }].map((s, i) => {
              const sp = spring({ frame: frame - 30 - i * 6, fps, config: { damping: 20 } });
              return (
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 24, flexWrap: "wrap" }}>
            {["Version History", "Approval Flow", "Template Library"].map((pill, i) => {
              const pillSpring = spring({ frame: frame - 50 - i * 6, fps, config: { damping: 20 } });
              return (
                <div
                  key={pill}
                  style={{
                    padding: "6px 14px",
                    borderRadius: 16,
                    background: "rgba(184,207,104,0.12)",
                    ...
                    opacity: pillSpring,
                    transform: `translateY(${interpolate(pillSpring, [0, 1], [10, 0])}px)`,
                  }}
                >
                  {pill}
                </div>
