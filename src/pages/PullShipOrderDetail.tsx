    ...
    try {
      ...
      if (order.parent_order_id) {
        const { invoiceNumber, percentageOfOrder } = await approvePullShipOrder({
          ...
          pullOrderItems: (editedOrder.order_items || []).map((item: any) => ({
            id: item.id,
            sku: item.sku,
        ...
        toast({
          ...
          description: invoiceNumber
            ? `Invoice ${invoiceNumber} created (${percentageOfOrder.toFixed(1)}% of blanket order)`
            : "Order approved successfully",
        });
      } else {
        toast({
          title: "Order Approved",
          description: "Order approved successfully (no parent order to invoice)",
        });
      }
