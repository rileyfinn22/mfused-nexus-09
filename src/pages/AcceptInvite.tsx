...
export default function AcceptInvite() {
  ...
  const validateInvitation = async () => {
    try {
      // Check if invitation is valid and if email already exists
      const { data, error } = await supabase.rpc('validate_company_invitation', {
      ...
      const result = data as { valid: boolean; email?: string; company_name?: string; role?: string; email_exists?: boolean; error?: string };
  ...
  const handleAcceptInvite = async (e: React.FormEvent) => {
    ...
    try {
      if (isLoginMode) {
        ...
        // New user - validate passwords and sign up
      ...
      toast({
        title: isLoginMode ? "Invitation accepted!" : "Account created!",
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-background to-muted">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            {isLoginMode 
              ? <>You already have an account. Log in to join <strong>{invitation?.company_name}</strong> as a {invitation?.role === 'admin' ? 'Company Admin' : 'Company User'}</>
          ...
          <form onSubmit={handleAcceptInvite} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                placeholder={isLoginMode ? "Enter your password" : "Min 8 chars, uppercase, number, special char"}
              />
              {!isLoginMode && (
            ...
            <Button type="submit" className="w-full" disabled={submitting}>
              ...
              {isLoginMode ? "Log In & Accept Invitation" : "Create Account & Join"}
            </Button>
          </form>
        </CardContent>
      </Card>
