...
Invoice Date: ${formatDocDate(invoice.invoice_date, 'numeric')}
Amount Due${depositLabel}: ${formatCurrency(amountDue)}
Due Date: ${invoice.due_date ? formatDocDate(invoice.due_date, 'numeric') : 'Upon Receipt'}
...
Thank you for your business.`;
  ...
  const generatePdfBase64 = async (): Promise<string> => {
    ...
    let termsY = drawTotals(doc, finalY + 5, {
      rows: totalsRows,
      grandLabel: 'BALANCE DUE',
      grandValue: formatCurrency(hasPayments ? balance : billedTotal),
      width: 85,
    }) + 14;

    // ============ TERMS/NOTES SECTION ============
    if (invoice.notes) {
      termsY = ensureRoom(doc, termsY, 24);
      termsY = drawNotes(doc, termsY, 'NOTES', invoice.notes) + 8;
    }

    termsY = ensureRoom(doc, termsY, 8);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'italic');
    doc.setTextColor(...DOC_COLORS.muted);
    doc.text('All remaining amounts are due on the agreed upon terms.', DOC.MARGIN, termsY);
            ...
            <div className="bg-muted/50 rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {invoice?.due_date ? formatDocDate(invoice.due_date, 'numeric') : "Upon Receipt"}
                </span>
              </div>
            </div>
