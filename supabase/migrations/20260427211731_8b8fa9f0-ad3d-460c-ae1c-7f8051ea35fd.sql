-- 1. Add blanket close tracking columns
ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS blanket_closed_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS blanket_closed_by UUID;

-- 2. Replace trigger with new logic:
--    - No children: per-line effective (NULL shipped → ordered, set shipped incl 0 → shipped × price)
--    - Has children + not closed: FROZEN (do not touch blanket subtotal/total)
...
BEGIN
  ...
  -- Ordered subtotal (qty × unit_price, always tracked on order record)
  ...
  -- Detect children + closed state on parent blanket
  SELECT EXISTS(
  ...
  IF v_has_children AND v_blanket_closed THEN
    -- Closed blanket: subtotal = Σ children
