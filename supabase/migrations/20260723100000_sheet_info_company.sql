-- Vendors see the attached customer company on their sheet. Piped through the
-- ownership-checked info RPC (name only). Return type changes — drop first.
...
AS $$
  ...
  left join public.companies c on c.id = p.customer_company_id
