-- Mark-complete on the vendor sheet: completed rows turn green and sink to a
-- "Completed" section at the bottom. Persisted as a timestamp so both sides
-- (admin + vendor) see the same state.

ALTER TABLE public.vendor_pos ADD COLUMN IF NOT EXISTS sheet_completed_at timestamptz;

-- Signature changes (new param) — drop the old overload first.
DROP FUNCTION IF EXISTS public.vendor_update_po_details(uuid, text, text, text, text, text, text, text, text, text, text, text, text);
...
grant execute on function public.vendor_update_po_details(uuid, text, text, text, text, text, text, text, text, text, text, text, text, boolean) to authenticated;
