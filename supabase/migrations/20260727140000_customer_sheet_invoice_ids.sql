-- Customers click the invoice number on the production sheet to open the
-- invoice page, so the sheet needs invoice ids. invoice_ids aligns 1:1 with
-- invoice_numbers (same filter + order). Return type changed -> drop first.
-- Applied live via claude-admin.

DROP FUNCTION IF EXISTS public.customer_production_sheet(uuid);

CREATE FUNCTION public.customer_production_sheet(p_company_id uuid)
